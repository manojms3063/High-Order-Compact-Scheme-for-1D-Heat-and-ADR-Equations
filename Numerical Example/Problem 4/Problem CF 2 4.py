...
B = 0.1
# Constants 
cb = (-beta + np.sqrt(beta**2 + 4*alpha*B))/(2*alpha)
...
# ax1.view_init(elev=10 , azim= 40)  # Change viewpoint
# # plt.savefig("numerical p3", dpi=600)
...
# ax2.view_init(elev=10 , azim= 40)  # Change viewpoint
# # plt.savefig("Exact p3", dpi=600)
...
# # ax.legend()
# plt.show()
# Plot only for the final time step T = 1
...
# Plot numerical and exact solutions at T = 1
ax.plot(x, u[:, final_step], 'o-', label="Numerical", color='r',linewidth=1.5, markersize=6)
ax.plot(x, v[:, final_step], 's--', label="Exact", color='b',linewidth=1.5, markersize=4)

ax.set_xlabel("Spatial Coordinate $x$")
ax.set_ylabel("Solution $u(x,t)$")
# ax.set_title("Comparison of Numerical and Exact Solutions at T=1")
ax.legend()
ax.grid(True)
# plt.savefig("Pe 1000 graph p3", dpi=600)
plt.show()
