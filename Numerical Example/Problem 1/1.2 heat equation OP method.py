...
for i in range(m+1):
    for j in range(n+1):
        v[i, j] = np.exp(-np.pi**2 * t[j]) * np.sin(np.pi * x[i])
...
# Select solution at final time step
u_num = u[:, -1]
u_ex = v[:, -1]

plt.figure(figsize=(8, 5))
plt.plot(x, u_num, 'ro-', label='Numerical', markersize=7, linewidth=1.5)
plt.plot(x, u_ex, 'b^--', label='Exact', markersize=5, linewidth=1.5)
...
plt.tight_layout()
# plt.savefig('opm_solution.png', dpi=700)
plt.show()
