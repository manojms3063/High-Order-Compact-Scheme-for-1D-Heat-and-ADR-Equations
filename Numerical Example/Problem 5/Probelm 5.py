...
# Define coefficients
S1 = -((alpha/h**2) + (beta**2 / (12 * alpha))-(gamma/12))
S2 = beta / (2 * h) - (gamma * beta * h) / (24 * alpha)
S3 = 1 / 12
S4 = - (beta * h) / (24 * alpha)
# S5 = -2*(alpha/h**2) - 2*(beta**2 / (12 * alpha))+2*(gamma/12) - gamma
S5 = 2*S1-gamma
...
u = np.zeros([m+1, n+1])
for i in range(0, m+1):
    u[i, 0] = sin(x[i]) #np.exp(-((x[i] - beta) ** 2) / (4 * alpha))
   
    
#Boundary Condition
for j in range(1, n+1):
    u[0, j] = exp(2 * t[j]) * np.sin(-t[j])
    u[m, j] = exp(2 * t[j]) * np.sin(x[m] - t[j])
...
for i in range(m+1):
    for j in range(n+1):
        v[i, j] = exp(2 * t[j]) * np.sin(x[i] - t[j])
...
ax1.view_init(elev=20 , azim= 30)  # Change viewpoint
# plt.savefig("numerical p2", dpi=600)
...
ax2.view_init(elev=20 , azim= 30)  # Change viewpoint
# plt.savefig("Exact p2", dpi=600)
# ax2.set_title("Exact Solution")
# plt.savefig("exact graph p5", dpi=600)
plt.show()
